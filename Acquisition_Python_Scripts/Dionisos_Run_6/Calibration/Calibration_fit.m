clear all
fast = 1;
scope_data = csvread('tek0004CH1.csv',21,0);
...
%     end
% end
scope_time_raw = flipud((scope_time_raw - min(scope_time_raw)));
scope_signal_raw = flipud(scope_signal_raw);
Bias_or_Current = 'Bias'



Data = csvread('MLP_test_data_Shot_9_Cal.csv',1,0);
...
if fast == 1
scope_parm = fit1(scope_time_raw(1:3:length(scope_time_raw)),scope_signal_raw(1:3:length(scope_time_raw)),3);
else
scope_parm = fit(scope_time_raw(1:3:length(scope_time_raw)),scope_signal_raw(1:3:length(scope_time_raw)),3);
end
